...
RUN apt-get update
RUN apt-get install -qy mysql-server mysql-client apache2 php5 php5-dev php5-gd php5-mysql subversion vim nano less mc sudo unoconv imagemagick ghostscript libgs-dev antiword xpdf libav-tools ffmpeg libimage-exiftool-perl cron wget poppler-utils zip python3-uno libreoffice-writer libreoffice-draw libreoffice-calc libreoffice-impress libreoffice-base-core libreoffice-core libreoffice-pdfimport libreoffice-avmedia-backend-gstreamer libreoffice-math libglu1-mesa
...
#scarico ed installo ResourceSpace versione 8.6
RUN mkdir /var/www/html.first \
    && cd /var/www/html.first \
	&& svn co http://svn.resourcespace.org/svn/rs/releases/8.6 . \
	&& mkdir filestore \
	&& chmod 777 filestore \
	&& chmod -R 777 include
